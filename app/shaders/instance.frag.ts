...
void main() {
	...
	float l = 0.5 + 0.4 * clamp(dot(vNormal, vec3(1.,1.,1.)), 0., 1.);
	// float diffuse = clamp(dot(vNormal, vec3(1.,1.,1.)), 0., 1.);
	// float specular = pow(dot(vNormal, vec3(0.,0.8,0.8)), 4.0);
	// float l = 0.5 + 0.5* diffuse + 0.5 * specular;
